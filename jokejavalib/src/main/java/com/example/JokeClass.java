...
public class JokeClass {
    ...
    private static Set<Joke> mUsedJokes = new HashSet<>();

    public Joke getJoke() {
        Joke joke;

        while (true) {
            if (mUsedJokes.size() == mJokes.size()) {
                joke = new Joke("No more jokes! And it's not a joke!!!");
                break;
            }
            joke = mJokes.get(new Random().nextInt(mJokes.size()));
            if (!mUsedJokes.contains(joke)) {
                mUsedJokes.add(joke);
                break;
            }
        }
